...
def autopad(k, p=None,d=1,s=1):  # kernel, padding
    # Pad to 'same'
    if k==3:
        p=1
    else:
...
class DecoupledHead(nn.Module):
    def __init__(self, ch=256, nc=80, anchors=()):
        ...
        nch=ch//2    
        self.cls_preds = nn.Conv2d(nch, self.nc * self.na, 1)
        self.reg_preds = nn.Conv2d(nch, 4 * self.na, 1)
        self.obj_preds = nn.Conv2d(nch, 1 * self.na, 1)
        
        self.merge = Conv(ch, nch, 1, 1)
        self.cls_convs1 = Conv(nch, nch, 3, 1, 1)
        self.cls_convs2 = Conv(nch, nch, 3, 1, 1)
        self.reg_convs1 = Conv(nch, nch, 3, 1, 1)
        self.reg_convs2 = Conv(nch, nch, 3, 1, 1)
...
class CBAM(nn.Module):
    def __init__(self, in_planes, k=0,ratio=16, kernel_size=7):
    ...
    def forward(self, x):
        # c=self.ca(x)
        # m=np.linspace(0,200,c.shape[1])
        # n=np.sort(c[0].view(-1))
        # plt.plot(m,n)
        # plt.show()
        out = x * self.ca(x) *2
        #s=
        result = out * self.sa(out)*2
        # # print(s.shape)
        # x=np.linspace(0,100,s.shape[2]*s.shape[3])
        # y=np.sort(s[0].view(-1))
        # plt.imshow( np.transpose(s[0], (1, 2, 0)))
        # plt.show()
        # plt.plot(x,y)
        # plt.show()
    ...
    def fuseforward(self,x):
        ...
        #print(self.k)
        #if self.k !=0:
            #k_val, k_ind = torch.kthvalue(s.view(-1),self.k)
        s1[s1 < 0.7] = 0
        #values, counts = torch.unique(s//0.01, return_counts=True)
        #print(values,counts)
        # st=s1[0].detach().numpy()
        # x=np.linspace(0,100,s1.shape[2]*s1.shape[3])
        # y=np.sort(st.reshape(-1))
        # plt.imshow( np.transpose(st, (1, 2, 0)))
        # plt.show()
        # plt.plot(x,y)
        # plt.show()
...
class Conv(nn.Module):
    ...
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, act=True, d=1):  # ch_in, ch_out, kernel, stride, padding, groups
        ...
        self.conv = nn.Conv2d(c1, c2, k, s, autopad(k, p, d,s), groups=g, dilation=d, bias=False)
